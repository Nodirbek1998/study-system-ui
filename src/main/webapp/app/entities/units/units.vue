  ...
  <div>
    ...
    <div class="table-responsive" v-if="units && units.length > 0">
      <table class="table table-striped" aria-describedby="units">
        ...
        <tbody>
          <tr v-for="units in units" :key="units.id" data-cy="entityTable">
            <td>
              <router-link :to="{ name: 'UnitsView', params: { unitsId: units.id } }">{{ units.id }}</router-link>
            </td>
            <td>{{ units.nameUz }}</td>
            <td>{{ units.nameRu }}</td>
            <td>{{ units.nameEn }}</td>
            <td>{{ units.createdAt }}</td>
            <td>{{ units.updatedAt }}</td>
            <td>
              <div v-if="units.subject">
                <router-link :to="{ name: 'SubjectsView', params: { subjectsId: units.subject.id } }">{{ units.subject.id }}</router-link>
              </div>
            </td>
            <td class="text-right">
              <div class="btn-group">
                ...
                <b-button
                  v-on:click="prepareRemove(units)"
                  variant="danger"
                  class="btn btn-sm"
...
<script lang="ts" src="./units.component.ts"></script>
