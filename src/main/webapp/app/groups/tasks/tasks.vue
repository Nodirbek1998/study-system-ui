  ...
  <div class="page page-internal">
    <h2 id="page-heading" data-cy="TaskHeading">
      ...
      <div class="d-flex justify-content-end">
        ...
        <router-link :to="{ name: 'CreateTask' }" custom v-slot="{ navigate }">
    ...
    <div class="table-wrapper" v-if="tasks && tasks.length > 0">
      ...
      >
        ...
        <template v-slot:head(files)="row">
          <span v-html="$t('studysystemApp.article.text')"></span>
        </template>
        ...
        <template v-slot:cell(files)="row"><span>{{ row.item.files ?  row.item.files.name : 'No' }}</span></template>
